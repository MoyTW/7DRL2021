      ...
      if (unit.StandingOrder == UnitOrder.REFORM) {
        ...
        if (this.AllowedActions(state, parent, unit.StandingOrder).Contains(actionMapping)) {
          if (actionMapping == InputHandler.ActionMapping.ROTATE) {
            ...
            parent.GetComponent<PlayerComponent>().AddPrestige(-2);
            state.LogMessage("You cry out [b]'Rotation!'[/b]' and, after a brief standoff with the foes opposite you, pull back through your lines. [b]You lose 2 prestige.[/b]");
            return null;
